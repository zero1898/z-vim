...
if [ "$(ls -al /bin/sh | awk '{print $11}')" != 'bash'  ]; then
    echo "your sh is not bash, push the password to change sh-->bash"
    echo "you also can use 'sudo dpkg-reconfigure dash' to change"
...
cleartrash(){
    ...
    [ $confirm == 'y' ] || [ $confirm == 'Y' ] && /bin/rm -rf $HOME/.local/share/Trash/files/*
}
